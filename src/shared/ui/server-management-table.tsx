...
export interface DashboardTableColumn<RowType> {
  key: string;
  header: string;
  className?: string;
...
export interface DashboardTableProps<RowType> {
  title?: string;
  subtitle?: string;
  columns: DashboardTableColumn<RowType>[];
  rows: RowType[];
  emptyMessage?: string;
  className?: string;
      ...
      >
        {(title || subtitle) && (
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-muted-foreground">Всего: {rows.length}</span>
          </div>
        )}
        ...
        >
          ...
          {/* Rows */}
          {rows.length === 0 && (
            <div className="px-4 py-6 text-sm text-muted-foreground text-center border border-dashed border-border/40 rounded-xl bg-muted/20">
              {emptyMessage}
            </div>
          )}

          {rows.map((row, index) => {
            const rowKey = getRowId ? getRowId(row, index) : `row-${index}`;
            ...
            return (
              ...
              >
                <motion.div
                  className="relative bg-muted/40 border border-border/50 rounded-xl p-4 overflow-hidden"
                  whileHover={{
                    y: -1,
...
export function ServerManagementTable({
  ...
  const handleStatusChange = (serverId: string, newStatus: Server['status']) => {
    ...
    setServers((prev) =>
      prev.map((server) => (server.id === serverId ? { ...server, status: newStatus } : server)),
    );
  };
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-muted-foreground">
              {servers.filter((s) => s.status === 'active').length} Servers •{' '}
              {servers.filter((s) => s.status === 'inactive').length} Servers
            </div>
          </div>
        </div>
